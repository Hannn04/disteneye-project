...
<div class="bg-sidebar vh-100 w-50 position-fixed">
    ...
    <table class="table student_list table-borderless">
        ...
        <tbody>
            ...
            $result = $connect->query("SELECT pesanan.*, user.nama FROM pesanan LEFT JOIN user ON pesanan.id_pemesan = user.id");
            ...
            ?>
                <tr class="bg-white align-middle">
                    ...
                    <td><?php echo $value['nama_depan_pelanggan'] . ' ' . $value['nama_belakang_pelanggan']; ?></td>
                    <td><?php echo $value['id_invoice'] ?></td>
                    <td><?php echo $value['destinasi'] ?></td>
                    <td><?php echo $value['alamat_email_pelanggan'] ?></td>
                    <td><?php echo $value['no_telepon_pelanggan'] ?></td>
                    <td><?php echo $value['tanggal_kedatangan'] ?></td>
                    <td>
                        <?php
                        if ($value['status'] == 0) {
                            echo "Belum Dibayar";
                        } elseif ($value['status'] == 1) {
                            echo "Perlu Di Verifikasi";
    ...
    <?php if ($value['status'] == 1): ?>
        ...
        <!-- <a href="../controller/proses_terima_pesanan.php?id_invoice=<?php echo $value['id_invoice'] ?>"><i class="far fa-check"></i></a>
        <a href="../controller/proses_tolak_pesanan.php?id_invoice=<?php echo $value['id_invoice'] ?>"><i class="far fa-times"></i></a> -->
        <!-- <a href="../controller/proses_edit_pesanan.php?id_invoice=<?php echo $value['id_invoice'] ?>"><i class="far fa-pencil"></i></a> -->
        <a href="../controller/proses_hapus_pesanan.php?id_invoice=<?php echo $value['id_invoice'] ?>"><i class="far fa-trash"></i></a>
